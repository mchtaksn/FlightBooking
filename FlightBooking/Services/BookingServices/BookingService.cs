...
public class BookingService : IBookingService
{
    ...
    {
       var booking = await _bookingCollection.Find(x=>x.Passengers.Any(x=>x.PassengerId==passengerId))
            .FirstOrDefaultAsync();
        if (booking == null)
            return null;
    ...
    public async Task<(string Name, string Surname)> GetPassengerNameByIdAsync(string passengerId)
    {
        ...
        if (passenger == null) return (null,null);
    ...
    {
        var booking = await _bookingCollection
            .Find(x => x.Passengers.Any(x => x.PassengerId == passengerId))
            .FirstOrDefaultAsync();
